...
class Dataset(object):
	def __init__(self, dataset_type):
		...
		self.users = pd.read_csv('data/u.user', sep='|', names=users_cols, encoding='latin-1')

		watch_cols = ['user_id', 'movie_id', 'watch_hist_time']
		self.watches = pd.read_csv('data/u.watch', sep='\t', names=watch_cols, encoding='latin-1')

		search_cols = ['user', 'search_hist']
		self.searches = pd.read_csv('data/u.search', sep='\t', names=search_cols, encoding='latin-1')

		ratings_cols = ['user_id', 'movie_id', 'rating', 'unix_timestamp']
		self.ratings = pd.read_csv('data/u.data', sep='\t', names=ratings_cols, encoding='latin-1')
