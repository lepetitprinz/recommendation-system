...
class CandidateGeneration(object):
    ...
    def build_nework(self):
        input_watch_hist = Input(shape=(None, ), name='watch_hist')
        input_watch_hist_time = Input(shape=(None,), name='watch_hist_time')
        input_search_hist = Input(shape=(None,), name='search_hist')
        input_example_age = Input(shape=(None, ), name='example_age')

        #--- layers
        features_embedding_layer = Embedding(input_dim=cfg.NUM_CLASSES, output_dim=cfg.EMBEDDING_DIMS, mask_zero=True,
                                             trainable=True, name='features_embeddings')
        labels_embedding_layer = Embedding(input_dim=cfg.NUM_CLASSES, output_dim=cfg.EMBEDDING_DIMS,mask_zero=True,
                                           trainable=True, name='labels_embeddings')

        avg_embeddings = MaskedEmbeddingsAggregatorLayer(agg_mode='mean', name='aggregate_embeddings')

        dense_1 = Dense(units=cfg.DENSE_UNITS, name='dense_1')
        dense_2 = Dense(units=cfg.DENSE_UNITS, name='dense_2')
        dense_3 =  Dense(units=cfg.DENSE_UNITS, name='dense_3')
